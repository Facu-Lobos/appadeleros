...
interface ClubRegistrationProps {
    onRegister: (profile: ClubProfileData, courts: CourtData[], photoFiles: File[]) => void;
    onBack: () => void;
}
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (clubName.trim() && courts.length > 0 && email.trim() && password.trim() && memberId.trim() && country && state && city) {
            ...
            const clubProfile: ClubProfileData = {
                ...
                openingDays,
                status,
                photos: [], // Will be filled by URLs from Supabase Storage
                notifications: [],
            };
            ...
            onRegister(clubProfile, newCourts, photoFiles);
        }
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                     ...
                     <div>
                        ...
                        <div className="p-4 bg-dark-primary/50 rounded-lg border border-dark-tertiary">
                           <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                                {photoFiles.map((photo, index) => (
                                    <div key={index} className="relative aspect-video group">
                                        <img src={URL.createObjectURL(photo)} alt={`Foto ${index+1}`} className="w-full h-full object-cover rounded-md" />
                                        <button type="button" onClick={() => removePhoto(index)} className="absolute top-1 right-1 bg-red-600/80 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity">
